...
def login_page_html(tid: str, icon: str = "🐾", biz_name: str = "Painel de Agendamentos", error: str = "") -> str:
...
async def create_appt(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if not all([customer_name, pet_name, service_id, scheduled_str]):
            return JSONResponse({"error": "Preencha todos os campos"}, status_code=400)
...
async def update_service(service_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    if "name" in data and data["name"].strip(): svc.name = data["name"].strip()
    db.commit()
    return {"success": True}
...
def export_relatorio(request: Request, mes: str = "", db: Session = Depends(get_db)):
    ...
    writer.writerow(["Data/Hora", "Cliente", "Pet", "Raça", "Peso(kg)", "Serviço", "Valor(R$)", "Status", "Pagamento", "Método Pgto", "Comprovante PIX", "Busca"])
    ...
    for a in appts:
        ...
        valor = f"{(a.payment_amount or (service.price if service else 0))/100:.2f}"
        status_label = STATUS_LABELS.get(a.status, (a.status, "", ""))[0]
        pay_label    = PAYMENT_LABELS.get(a.payment_status or "pending", (a.payment_status, "", ""))[0].replace("💳","").replace("✅","").replace("🎁","").strip()
        writer.writerow([
...
def dashboard(request: Request, tid: str = "", db: Session = Depends(get_db)):
    ...
    # Verifica se é primeiro acesso (sem serviços cadastrados = onboarding)
    has_services = db.query(Service).filter(Service.tenant_id == tid, Service.active == True).count() > 0
    has_appts    = db.query(Appointment).filter(Appointment.tenant_id == tid).count() > 0
    ...
    historico = db.query(Appointment).filter(Appointment.tenant_id == tid).order_by(Appointment.scheduled_at.desc()).limit(100).all()
    ...
    total_agendamentos = db.query(Appointment).filter(Appointment.tenant_id == tid, Appointment.status.in_(["confirmed","in_progress","ready","delivered"])).count()
    ...
    fat_mes = sum((a.payment_amount or (db.query(Service).filter(Service.id == a.service_id).first() or type('',(),{'price':0})()).price or 0) for a in appts_mes_pagos)
    ...
    fat_pendente = sum((db.query(Service).filter(Service.id == a.service_id).first() or type('',(),{'price':0})()).price or 0 for a in pendentes_mes)
    ...
    top_svc = ""
    if svc_ids:
        top_s = db.query(Service).filter(Service.id == Counter(svc_ids).most_common(1)[0][0]).first()
    ...
    if show_onboarding:
        ...
        steps = [
            ...
            ("✅" if wa_configured else "⬜", "Configure o WhatsApp no painel Admin (Phone Number ID + Token)"),
            ("✅" if owner_configured else "⬜", "Adicione seu número no Admin para receber notificações de novos agendamentos"),
            ("⬜", "Faça seu primeiro agendamento clicando em <strong>+ Agendar</strong>"),
        ]
        steps_html = "".join(f'<div style="display:flex;gap:10px;align-items:flex-start;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px"><span style="font-size:16px">{s}</span><span style="color:var(--text2)">{t}</span></div>' for s,t in steps)
        onboarding_html = f"""
        <div class="card" style="border-color:var(--accent);background:var(--accent-bg)">
            <div class="section-title" style="color:var(--accent)">🚀 Bem-vindo! Configure sua agenda em 4 passos</div>
            {steps_html}
        </div>"""
    ...
    def _payment_btn(a, svc):
        ...
        return f'<button class="pay-btn" onclick="openPayModal(\'{a.id}\', {price_val})" style="background:{pbg};color:{pc};border:1px solid {pc}33;font-size:10px;padding:3px 8px;border-radius:6px;cursor:pointer;font-family:\'DM Sans\',sans-serif;font-weight:700">{pl}</button>'
    ...
    if not agendamentos_hoje:
        ...
        for a in agendamentos_hoje:
            ...
            status_options = "".join(f'<option value="{k}" {"selected" if a.status==k else ""}>{sl}</option>' for k,(sl,sb,sc) in STATUS_LABELS.items() if k!="cancelled")
            ...
            <div class="appt-card" id="card-{a.id}">
                ...
                <div class="appt-body">
                    ...
                    <div class="appt-pet">{tenant_icon} {pet_info}</div>
    ...
    rows_proximos = ""
    if not proximos:
        rows_proximos = '<tr><td colspan="9" class="empty-row">Nenhum agendamento nos próximos 7 dias.</td></tr>'
    else:
        for a in proximos:
            customer = db.query(Customer).filter(Customer.id == a.customer_id).first()
            service  = db.query(Service).filter(Service.id == a.service_id).first()
            ...
            pl,pbg,pc = PAYMENT_LABELS.get(ps,("💳 Pend.","#fff8e1","#c67d00"))
            rows_proximos += f"""<tr>
                <td>{a.scheduled_at.strftime("%d/%m %H:%M")}</td><td>{nome_cliente}</td>
                <td>{a.pet_name or '-'}</td><td>{a.pet_breed or '-'}/{f"{a.pet_weight}kg" if a.pet_weight else '-'}</td>
                <td>{nome_servico}</td><td>{a.pickup_time or '-'}</td>
    ...
    pendentes_all = db.query(Appointment).filter(
        Appointment.tenant_id == tid, Appointment.payment_status == "pending",
        Appointment.status != "cancelled"
    ...
    if not pendentes_all:
        ...
        for a in pendentes_all:
            ...
            rows_pendentes += f"""<tr>
                <td>{a.scheduled_at.strftime("%d/%m/%Y %H:%M")}</td><td>{nome_cliente}</td>
                <td>{a.pet_name or '-'}</td><td>{nome_servico}</td>
    ...
    for s in services_all:
        ...
        <div class="service-edit-row" id="srow-{s.id}">
            ...
            <div style="font-size:12px;color:var(--text3)">{s.description or ''}</div></div>
            {active_badge}
    ...
    service_options = "".join(f'<option value="{s.id}">{s.name} — R$ {s.price/100:.2f}</option>' for s in services_all if s.active)
    ...
    bot_badge  = '<span class="badge badge-green">🤖 Bot ativo</span>' if bot_status else '<span class="badge badge-red">🤖 Pausado</span>'
...
<style>
:root[data-theme="light"]{{--bg:#f4f6fb;--surface:#ffffff;--surface2:#f8f9fc;--border:#e8ecf2;--text:#1a1d23;--text2:#5a6172;--text3:#9aa0b0;--accent:#5B5BD6;--accent2:#7c7de8;--accent-bg:#ededfc;--shadow2:rgba(0,0,0,0.12);--header-bg:#1a1d23;--header-text:#ffffff;--danger:#e53e3e;--danger-bg:#fff5f5;--success:#2e7d32;--success-bg:#e8f5e9;--warn:#c67d00;--warn-bg:#fff8e1;--info:#1565c0;--overlay:rgba(0,0,0,0.4);--input-bg:#f8f9fc;--modal-bg:#ffffff;}}
:root[data-theme="dark"]{{--bg:#0f1117;--surface:#1a1d27;--surface2:#22263a;--border:#2d3148;--text:#e8eaf2;--text2:#9aa0b8;--text3:#5a6172;--accent:#7c7de8;--accent2:#9c9df0;--accent-bg:#23254a;--shadow2:rgba(0,0,0,0.5);--header-bg:#13151f;--header-text:#e8eaf2;--danger:#fc8181;--danger-bg:#2d1515;--success:#68d391;--success-bg:#1a2e1a;--warn:#f6c90e;--warn-bg:#2a2200;--info:#63b3ed;--overlay:rgba(0,0,0,0.7);--input-bg:#1a1d27;--modal-bg:#1a1d27;}}
*{{margin:0;padding:0;box-sizing:border-box}}
...
.btn-primary{{background:var(--accent);color:white;border:none;padding:8px 14px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:600;font-family:'DM Sans',sans-serif;display:flex;align-items:center;gap:5px}}
...
.stat-card.warn{{border-color:#c67d0060;background:var(--warn-bg)}}.stat-card.warn .stat-number{{color:var(--warn)}}
...
.modal{{background:var(--modal-bg);border-radius:18px;padding:26px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px var(--shadow2);border:1px solid var(--border);transform:translateY(20px);transition:transform .25s;margin:20px}}
...
<div class="header">
    <div class="header-logo"><span class="icon">{tenant_icon}</span> <span>{tenant_name}</span></div>
    <div class="header-right">
        ...
        <a href="/api/export/relatorio?mes={mes_atual}" class="btn-icon" title="Exportar relatório {mes_label}" style="text-decoration:none">📥</a>
...
<div class="tabs">
    ...
    <button class="tab"         onclick="switchTab('pendentes',this)">⏳ Pagamentos <span class="badge-count">{len(pendentes_all)}</span></button>
...
<div id="tab-proximos" class="tab-content">
    <div class="card">
        ...
        <div class="table-wrap"><table>
            <thead><tr><th>Data/Hora</th><th>Cliente</th><th>{subject}</th><th>Raça/Peso</th><th>Serviço</th><th>Busca</th><th>Status</th><th>Pgto</th><th></th></tr></thead>
...
<div id="tab-historico" class="tab-content">
    <div class="card">
        <div class="section-title" style="justify-content:space-between">
            ...
            <a href="/api/export/relatorio?mes={mes_atual}" class="btn-save-svc" style="font-size:11px;padding:4px 10px;text-decoration:none">📥 Exportar {mes_label}</a>
        </div>
        <div class="search-box">
            <input class="search-input" id="search-input" placeholder="🔍 Buscar por cliente ou {subject.lower()}..." oninput="filterTable()">
        </div>
        <div class="table-wrap"><table>
            <thead><tr><th>Data/Hora</th><th>Cliente</th><th>{subject}</th><th>Serviço</th><th>Valor</th><th>Status</th><th>Pgto</th><th>Criado em</th></tr></thead>
...
<div id="tab-servicos" class="tab-content">
    <div class="card">
        ...
        <div class="add-svc-form">
            ...
            <div class="form-group" style="margin-top:10px;margin-bottom:0"><label>Descrição (para o bot)</label><input id="ns_desc" placeholder="Ex: Corte com acabamento e finalização"></div>
        </div>
    </div>
</div>
...
<div class="modal">
    ...
    <div class="form-row">
        <div class="form-group"><label>{tenant_icon} {subject} *</label>
...
<!-- Modal: Pagamento com revisão PIX -->
...
<div class="modal">
    ...
    <input type="number" id="pay_amount" step="0.01" placeholder="0.00"></div>
    ...
    <div id="pix_section" style="display:none">
        <div class="pix-section">
            ...
            <input type="text" id="pay_pix_key" placeholder="Cole o ID/código do comprovante PIX">
            <div style="font-size:11px;color:var(--success);margin-top:6px">💡 Cole o ID do comprovante para rastreio e auditoria</div>
        </div>
        <div class="pix-review-box">
            ⚠️ <strong>Revisão obrigatória:</strong> Confira no seu app bancário se o PIX foi realmente recebido antes de confirmar. Nunca confirme baseado apenas na imagem enviada pelo cliente.
        </div>
    </div>
    <div class="form-group" style="margin-top:12px"><label>Observação</label>
    <input type="text" id="pay_notes" placeholder="Ex: Desconto aplicado, pagou adiantado..."></div>
...
function selectSlot(time,btn){{
    ...
    btn.classList.add('selected');selectedTime=time;
...
async function submitAppt(){{
    ...
    if(!customer||!pet||!service_id||!date||!time){{showToast('⚠️ Preencha todos os campos e escolha um horário');return;}}
...
async function confirmPayment(status){{
    ...
    if(status==='paid'&&method==='pix'&&!pixKey){{
        if(!confirm('⚠️ Você confirmou o recebimento no seu banco?\\n\\nNão confirme sem verificar o extrato.'))return;
    }}
    try{{
...
async function saveService(id){{
    ...
    const d=await r.json();
    if(d.success)showToast('✅ Serviço salvo!');else showToast('❌ Erro');
}}
