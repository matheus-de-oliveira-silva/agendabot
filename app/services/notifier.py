...
async def configure_instance_webhook(instance_name: str, webhook_url: str = "") -> bool:
    ...
    Funciona com v1, v2 e v2.3.x automaticamente.
    """
    ...
    # Todos os formatos conhecidos da Evolution API — testa até um funcionar
    payloads = [
        # Formato v2.3.x — camelCase com enabled
        {
            "enabled":         True,
            "url":             webhook_url,
        ...
        # Formato v2 — snake_case com enabled
        {
            "enabled":          True,
            "url":              webhook_url,
        ...
        # Formato v2 — aninhado em "webhook"
        {
            "webhook": {
                "enabled":         True,
                "url":             webhook_url,
        ...
        for i, payload in enumerate(payloads, 1):
            try:
                ...
                if resp.status_code in (200, 201):
                    ...
                    print(f"[Evolution] formato {i} falhou {resp.status_code}: {resp.text[:60]}")
            except Exception as e:
